...
def word_format(str)
  raw_word = str.split.reverse
        alias_step = next_consonant(next_vowel(raw_word))
        #Format alias name
        alias_final = alias_step.split.map(&:capitalize).join(' ')
        p "#{alias_final} is an alias for #{str}"
end
...
until input_name == "quit" || input_name == ""
    puts "What is your first and last name? Type 'quit' to exit."
      ...
      format_name = input_name.split.reverse.join(' ')
      if input_name == "quit" || input_name == ""
        puts "Exiting Alias Generator"
      else
        #Switch vowels
        alias_step1 = next_vowel(format_name)
        #Switch consonants
